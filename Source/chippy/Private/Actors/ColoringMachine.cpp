	...
	if (HasAuthority() && !AvailableColors.IsEmpty())
	{
		for (int i = 0; i < AvailableColors.Num(); i++)
		{
			if (MachineButtonClass)
			{
				...
				FVector Loc = ButtonsSpawnLocation->GetComponentLocation() + (ButtonsSpawnLocation->GetForwardVector() *
					(i * ButtonsPadding));
...
void AColoringMachine::StartWorkingOnProduct(FProductInfo ProductToCreate)
{
